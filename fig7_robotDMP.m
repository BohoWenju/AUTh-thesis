...
base_pos(2) = base_pos(2) - 0.3;
base_pos(1) = base_pos(1) + 0.3 ;
...
temp = robot_obj.fkine(q0);
angle_of_rotation = acos(make_unitary(g-y0)'*temp(1:3,1));
q0 = q0 + [0; 0; 0; 0; 0; 0; angle_of_rotation];
...
for t = 0:dt:T
    ...
    [q(:,i+1), dq(:,i+1), ddq(:,i), xe(:,i+1), Q(:,i+1)] = controller_obj.simulation(...
...
x_axis = R(1:3,1);
for i = 1:timesteps
    ndy = make_unitary(dy(:,i));
    theta = acos(x_axis'*ndy);
    ne = get_skew(x_axis)*ndy;
    eo = theta*ne;
    w = -400*eo;
    Q_dot = 1/2*get_JQ(Q(:,i))*w;
    Q(:,i+1) = Q_dot*dt + Q(:,i);
    R = quat2rotm(Q(:,i+1)');
    x_axis = R(1:3,1);
    i = i+1;
...
k = y0 + s.*(gn(:,2)-y0);
q_then = q(:,time_moments(2));
% temp = lwr4_fkine(q_then);
temp = quat2rotm(Q(:,time_moments(2))');
temp = [temp y(:,time_moments(2)); zeros(1,3) 1];
y_axis = temp(1:3,2);
temp = Hb*temp;
k1 = temp(1:3,4)+s.*(y_axis*0.2 - temp(1:3,4));
figure();
plot(k1(1,:),k1(2,:),'LineWidth',2, 'LineStyle','--','Color','green','DisplayName','Vertical Line');
hold on
plot(k(1,:),k(2,:),'LineWidth',2, 'LineStyle','-','Color','blue','DisplayName','Forearm');
hold on
plot(k1(1,end),k1(2,end), 'LineStyle','None', 'Marker','o', 'Color',[1 0 0.2], 'LineWidth',3, 'MarkerSize',10, 'HandleVisibility','off');
hold on
plot(k(1,1),k(2,1), 'LineStyle','None', 'Marker','o', 'Color',[1 0.6 0.2], 'LineWidth',3, 'MarkerSize',10, 'HandleVisibility','off');
hold on
plot(k(1,end),k(2,end), 'LineStyle','None', 'Marker','x', 'Color',[1 0.6 0.2], 'LineWidth',3, 'MarkerSize',10, 'HandleVisibility','off');
grid on
axis equal
